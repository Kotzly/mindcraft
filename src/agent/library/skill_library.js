...
export class SkillLibrary {
    ...
    async getRelevantSkillDocs(message, select_num) {
        ...
        if (select_num === -1) {
            skill_doc_similarities = this.skill_docs.map(doc_key => ({ doc_key, similarity_score: 0 }));
        }
        else if (!this.embedding_model) {
            skill_doc_similarities = this.skill_docs
                .map(doc_key => ({ doc_key, similarity_score: wordOverlapScore(message, doc_key) }))
                .sort((a, b) => b.similarity_score - a.similarity_score);
        }
        else {
            let latest_message_embedding = await this.embedding_model.embed(message);
            skill_doc_similarities = this.skill_docs
                .filter(doc_key => this.skill_docs_embeddings[doc_key])  // skip docs whose embedding failed
                .map(doc_key => ({ doc_key, similarity_score: cosineSimilarity(latest_message_embedding, this.skill_docs_embeddings[doc_key]) }))
                .sort((a, b) => b.similarity_score - a.similarity_score);
        }
